...
public class FormatTransformerServiceTest {
    ...
    public void convertingIssuesToJsonWorks() throws Exception {
        ...
        assertNull(testReq.getComments());
        assertEquals(testReq.getRequirement_type(), "Suggestion");
        assertEquals(testReq.getStatus(), "Reported");
    ...
    public void jsonGetsCorrectFixVersion() throws Exception {
        ...
        RequirementPart part = reqList.get(2).getRequirementParts()
                .stream().filter(p -> p.getName().equals("FixVersion")).findFirst().get();
        assertEquals(part.getText(), "Qt Creator 4.9.0-beta1");
        part = reqList.get(3).getRequirementParts()
                .stream().filter(p -> p.getName().equals("FixVersion")).findFirst().get();
        assertEquals(part.getText(), "Qt Creator 4.9.0 (4.9 branch)");
    }
    ...
    public void testWithPlatforms() throws Exception {
        ...
        Set<RequirementPart> parts = reqList.get(0).getRequirementParts();
        RequirementPart part = parts.stream().filter(p -> p.getName().equals("Platforms")).limit(1).findFirst().orElse(null);
    ...
    public void testMakingIssuesWithTypesAndStatuses() throws Exception {
        ...
        // TODO all of these override each other since they have the same key. why????
        ...
        List<Requirement> reqList = new ArrayList<Requirement>(reqCollection);
        Requirement testReq = reqList.get(0);
        assertEquals(testReq.getStatus(), "done");
    }
