...
do
	...
	# # Get data from Jira to Mallikas
	 ...
	 start=`date +%s`
	 curl -X POST --header 'Content-Type: application/json' --header 'Accept: text/plain' -d $i 'localhost:9203/qtjira' 
	 end=`date +%s`
	...
	start=`date +%s`
	curl -X POST --header 'Content-Type: application/json' --header 'Accept: text/plain' -d $i  'localhost:9203/sendProjectToMulperi' 
	end=`date +%s`
